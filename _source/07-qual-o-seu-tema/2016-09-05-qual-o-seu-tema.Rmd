...
```{r}
contagem %>%
  #filter(ymd(time) < ymd(20150101), ymd(time) >= ymd(20140101), main_theme != "administração") %>%
  filter(main_theme != "administração") %>%
  streamgraph(key = "main_theme", value = "count", date = "time", offset = "expand") %>%
  sg_fill_brewer("PuOr") %>%
